import React from "react";
import { Button } from "components/button";
import { InfoIcon } from "components/icons";

export const START_REVIEW_BUTTON_NAME = "button-start-review";
export const DELIVERABLE_REVIEW_NOTICE_NAME = "deliverable-review-notice";

export const PendingReviewNotice: React.FC<{
  submitterName: string;
  onStartReview: () => void;
  isSubmitting?: boolean;
}> = ({ submitterName, onStartReview, isSubmitting = false }) => {
  return (
    <div
      className="flex items-center gap-2 border border-brand border-l-[6px] rounded-sm bg-white px-1 py-1"
      data-testid={DELIVERABLE_REVIEW_NOTICE_NAME}
    >
      <span className="shrink-0" aria-hidden="true">
        <InfoIcon width="28" height="28" />
      </span>
      <div className="flex-1 leading-2">
        <p className="text-[15px] font-bold text-text-font">Submission pending review</p>
        <p className="text-sm text-text-font">
          {submitterName} has submitted deliverable(s) for review
        </p>
      </div>
      <Button
        type="button"
        size="small"
        name={START_REVIEW_BUTTON_NAME}
        onClick={onStartReview}
        disabled={isSubmitting}
      >
        Start Review
      </Button>
    </div>
  );
};
