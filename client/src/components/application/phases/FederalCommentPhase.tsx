...
const FederalCommentNotice = ({
  noticeDueDate,
  federalCommentComplete,
}: {
  noticeDueDate: string;
  federalCommentComplete: boolean;
}) => {
  useEffect(() => {
    setNoticeDismissed(federalCommentComplete === true);
  }, [federalCommentComplete]);

  const [isNoticeDismissed, setNoticeDismissed] = useState(
    federalCommentComplete === true
  );

  useEffect(() => {
    setNoticeDismissed(federalCommentComplete);
  }, [federalCommentComplete]);

  const noticeDueDateValue = parseInputDate(formatDateForServer(new Date(noticeDueDate)));
  ...
  return (
    <div>
      <FederalCommentNotice
        noticeDueDate={phaseEndDate}
        federalCommentComplete={phaseComplete}
      />
