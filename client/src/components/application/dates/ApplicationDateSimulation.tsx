...
export const ApplicationDateSimulation: React.FC = () => {
  ...
  const startPhase = (phaseName: PhaseName, startDateType: DateType) => {
    ...
    const updatedDates = setDateInApplicationDates(
      simulationState.applicationDates,
      startDateType,
      now
    );
    setSimulationState({ phases: updatedPhases, applicationDates: updatedDates });
  };
  ...
  const completePhase = (
    ...
    let updatedDates = simulationState.applicationDates;

    if (completionDateType) {
      updatedDates = setDateInApplicationDates(updatedDates, completionDateType, now);
    }
    ...
    if (nextPhaseName && nextPhaseStartDateType) {
      ...
      updatedDates = setDateInApplicationDates(updatedDates, nextPhaseStartDateType, now);
    }
  ...
  const skipPhase = (
    ...
    let updatedDates = setDateInApplicationDates(
      simulationState.applicationDates,
      completionDateType,
      now
    );
  ...
  const submitChange = (
    ...
    if (autoStartPhase && phaseStatus === "Not Started") {
      ...
      updatedDates = setDateInApplicationDates(updatedDates, startDateType, now);
    }

    // Set the specified date if provided
    if (dateToSet) {
      updatedDates = setDateInApplicationDates(updatedDates, dateToSet, now);
    }
    ...
    if (phaseName === "State Application") {
      ...
      if (conceptStatus === "Not Started" || conceptStatus === "Started") {
        ...
        updatedDates = setDateInApplicationDates(updatedDates, "Concept Completion Date", now);
      }
      ...
      if (dateToSet === "State Application Submitted Date") {
        ...
        if (completenessStatus === "Not Started") {
          ...
          updatedDates = setDateInApplicationDates(updatedDates, "Completeness Start Date", now);
        }
      }
    }
  ...
  const isStateApplicationSubmittedDateSet = () => {
    const submittedDate = getDateFromApplicationDates(
      simulationState.applicationDates,
