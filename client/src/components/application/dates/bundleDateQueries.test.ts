...
describe("phaseDateQueries", () => {
  describe("getQueryForSetBundleDate", () => {
    it("should generate correct GraphQL mutation for Start Date", () => {
      const result = getQueryForSetBundleDate(TEST_SET_PHASE_DATE_INPUT);

      expect(result).toContain("mutation SetBundleDate");
    ...
    it("should handle special characters in bundleId", () => {
      const input: SetBundleDateInput = {
        ...TEST_SET_PHASE_DATE_INPUT,
        bundleId: "bundle-with-special-chars-123_abc",
      };

      const result = getQueryForSetBundleDate(input);
    ...
    it("should maintain GraphQL mutation structure", () => {
      ...
      expect(result).toMatch(/mutation SetBundleDate\s*{/);
      expect(result).toMatch(/setBundleDate\s*\(\s*input:\s*{/);
