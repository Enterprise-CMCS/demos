...
import { ADD_DEMONSTRATION_DOCUMENT_MUTATION, UPDATE_DEMONSTRATION_DOCUMENT_MUTATION, DELETE_DEMONSTRATION_DOCUMENT_MUTATION, ADD_AMENDMENT_DOCUMENT_MUTATION, UPDATE_AMENDMENT_DOCUMENT_MUTATION, DELETE_AMENDMENT_DOCUMENT_MUTATION, GET_DOCUMENT_QUERY, GET_DOCUMENTS_QUERY } from "queries/document/documentQueries";
import { Document, CreateDemonstrationDocumentInput, UpdateDemonstrationDocumentInput, BundleType } from "demos-server";

export interface DocumentOperations {
  createDemonstrationDocument: (input: CreateDemonstrationDocumentInput) => Promise<FetchResult<{ addDemonstrationDocument?: Document }>>;
...
export const useDocument = (): DocumentOperations => {
  ...
  return {
    createDemonstrationDocument: (input) => addDemonstrationDocumentTrigger({ variables: { input } }),
