// Generated from runbooks/revalidate-jsonb.md by docs/tools/md_to_adoc.py -- do not edit.
...
. *Count offending rows* via the helper:
+
[,sql]
----
...
. *If the count is non-zero*, list the offenders so the SME can
triage:
+
[,sql]
----
...
. *Re-run the helper* until it returns `0` for every column you
touched.
