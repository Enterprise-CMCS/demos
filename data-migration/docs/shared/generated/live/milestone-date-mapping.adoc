...
| `mdcd_demo_aplctn.phase_3_a_sme_strt_dt` / `phase_3_a_sme_end_dt`
| SME clearance sub-dates; candidate target `SME Initial Review Date` is not high-confidence (start vs end, and which of several SDG-prep sub-dates).

| `mdcd_demo_aplctn.phase_3_a_frvt_*`, `phase_3_b_cmcs_*`, `phase_3_b_ogc_*`, `phase_3_b_omb_*`, `phase_3_c_ogc_*`, `phase_3_c_omb_*` (start+end)
| FRT / BNPMT / OGC / OMB clearance sub-dates; their DEMOS targets (`FRT Initial Meeting Date`, `BNPMT Initial Meeting Date`, and the Review-phase clearances) require SME ratification of the ordinal/semantic match. Their earliest start already feeds `SDG Preparation Start Date`.
