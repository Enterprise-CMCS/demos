...
through `#workflow-11`). The decision table above records _entity_ rules;
...
read the same source column as a _different_ DEMOS date type than this pipeline
...
disagree about what the phase _is_:
...
this is a question about what PMDA's phases _mean_, which only the SME can
...
. *The six colliding mappings above.* Until the SME rules, this pipeline
keeps its existing reading unchanged. Whichever way it goes, one of the two
migrations is mislabelling six dates today.
. *`phase_3_c_ogc_strt_dt` / `phase_3_c_omb_strt_dt`.* dbt maps these to
`'Receive OGC Legal Clearance'` and `'Receive OMB Concurrence'`. Both are
`*_strt_dt` columns being read as _receipt_ dates, i.e. the end of the
...
. *Three dbt date types with no source column at all.* dbt synthesizes
...
. *The `Skipped` phase status.* DEMOS models a skipped Concept phase as a
...
status is deferred _together with_ item 3 and stands or falls with it.
...
* [ ] SME confirms whether `phase_3_c_*_strt_dt` are clearance _receipt_ dates.
