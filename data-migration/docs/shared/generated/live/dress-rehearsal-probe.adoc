// Generated from runbooks/dress-rehearsal-probe.md by docs/tools/md_to_adoc.py -- do not edit.
...
(W5/W6) into a concrete fix list. This is _not_ the timed, zero-intervention
Tier-2 rehearsal and _not_ a real cutover (no `flip`/`smoke`/`decom`).
...
``SELECT``s against MySQL and compares to Postgres, so a _moving_ source would
...
cutover chain must run. `freeze` only _records_ the instant (it does not pause
