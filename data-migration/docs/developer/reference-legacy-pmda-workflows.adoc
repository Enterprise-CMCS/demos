= Reference: legacy PMDA workflows
:toc: left

PMDA is the legacy application DEMOS replaces. This page is the surviving
description of its shape: eleven data workflows, the PHP repository class
behind each one, and the MySQL extract queries that pull each workflow out of
the PMDA source schema. It was reconstructed by reading PMDA's `cma-site` (UI
routes and actions) and `cma-service` (data workflows) repositories. By the end
you will know which source workflow owns which `+mdcd_*+` tables, and what a
faithful extract of each one looks like.

[IMPORTANT]
====
This page documents the *source* system, not the target. It was reconstructed
by reading PMDA's `cma-site` and `cma-service` repositories, which are not
present in this tree, so the claims here cannot be re-derived from this
repository alone. It entered this repository on 2026-07-10 and has not been
re-verified against PMDA since. Treat it as dated evidence: useful for
understanding why a migration decision was made, not as a live description of
PMDA today.
====

== How PMDA is organised

PMDA is workflow-centric, not table-centric. The useful map is `cma-site` for
UI routes/actions and `cma-service` for the actual data workflows. In practice,
the SQL summary should be based on the backend repositories, especially
`cma-service/routes/web.php` and the repository classes under
`cma-service/app/Repositories`.

== The eleven migration workflows

These are the most important migration workflows, in the source document's
order. The numbering is load-bearing: the workflow-level scope dispositions in
xref:../sme/reference-pending-approved-rules.adoc[the pending/approved rules]
are keyed to it. What follows describes the source system only; what this
project decided to do about each workflow is recorded there, not here.

[[workflow-1]]
=== 1. Reference data

Statuses, demo types, document types, roles, states, occurrence types,
categories, reasons, and determination codes. PMDA logic depends heavily on
numeric/code values, so do not blindly remap them without a crosswalk.

[[workflow-2]]
=== 2. Users, roles, and state access

*Source:* `cma-service/app/Repositories/DbUserRepository.php`

Important tables include `users`, `user_role_asgnmt`, and
`user_authrzd_state_acs`. Preserve inactive/deleted users too if they are
referenced by demos, comments, deliverables, emails, or histories.

[[workflow-3]]
=== 3. Approved demonstrations

*Source:* `cma-service/app/Repositories/DbDemonstrationRepository.php`

Core table is `mdcd_demo`. The important related data is status history,
performance period history, comments, contacts, amendments,
renewals/extensions, final decision info, site visits, post-award forums, and
authority documents.

[[workflow-4]]
=== 4. Programs and demo types

*Source:* `cma-service/app/Repositories/DbProgramRepository.php`

Core table is `mdcd_pgm`, but demo-type details live in many type-specific
tables. Views like `v_demo_mgmt_demo_types` are useful for identifying
relationships, but the migration should still pull the underlying base rows.

[[workflow-5]]
=== 5. Waiver and expenditure authorities

These are part of the demo/program domain, not optional metadata. Important
families include waiver authority tables, expenditure authority tables,
not-applicable expenditure authority tables, and demo-link tables. Preserve
expiration dates, deleted flags, and section/title/category relationships.

[[workflow-6]]
=== 6. Deliverables

*Source:* `cma-service/app/Repositories/DbDeliverableRepository.php`

Core table is `mdcd_dlvrbl`, but the real workflow includes status history,
comments by origin, uploaded files, paper records, due-date-change requests,
determinations, template links, BN/MRT flags, and processed-file flags.
Migrating only deliverable rows would lose a lot of behavior.

[[workflow-7]]
=== 7. Application management / pending demos

*Sources:* `cma-service/app/Repositories/DbAppMgmtDemoRepository.php`,
`cma-service/app/Repositories/DbAppMgmtProgramRepository.php`

This covers pending demos, applications, amendments/extensions, pending
programs, application comments, federal decisions, and uploaded application
documents. Pending application data is separate from approved-demo data and
should be treated as its own migration area.

[[workflow-8]]
=== 8. Budget Neutrality

*Source:* `cma-service/app/Repositories/DbBudgetNeutralityRepository.php`

BN is file-backed plus parsed-data-backed. Important data includes template
files, deliverable workbook files, parsed demo years, MEGs, populations,
waivers, PMPM/cost data, warnings, totals, and division dashboard rows.

[[workflow-9]]
=== 9. Monitoring Reporting Tool

*Source:*
`cma-service/app/Repositories/DbMonitoringReportingToolRepository.php`

Similar to BN: uploaded templates/files plus parsed workbook/report/protocol
data. Also uses demo-level MRT flags and start dates on `mdcd_demo`.

[[workflow-10]]
=== 10. STC documents

*Source:* `cma-service/app/Repositories/DbSTCRepository.php`

This is a document/search/cart workflow around Special Terms and Conditions.
Migrate it if Demos needs STC search, generated docs, bookmarks, or cart
history.

[[workflow-11]]
=== 11. Email, reports, help, support, and admin

*Sources:* `cma-service/app/Repositories/DbEmailRepository.php`,
`cma-service/app/Repositories/DbReportRepository.php`

These may be archive-only depending on Demos scope. Email history has recipient
tables. Help/support, FAQs, notification bars, params, and error logs are
admin/reporting data.

== Source extract queries

The safest migration shape is:

. Load reference/code tables. See `+*_rfrnc+` tables

. Load users/roles/state access.
+
[source,sql]
----
 SELECT
...
. Load approved demos and preserve original PMDA IDs or maintain explicit ID
crosswalks.
+
In PMDA, "approved demos" means the approved-demonstration side of the app:
source rows from `mdcd_demo`. That is broader than only
`mdcd_demo_stus_cd = 2`. Use `dltd_ind = 0` for the active migration scope; add
`AND d.mdcd_demo_stus_cd = 2` only if the target should receive currently
approved-only rows.
+
[source,sql]
----
...
----
+
Demos uses UUIDs for `demos_app.demonstration.id`, so do not try to insert
PMDA's integer `mdcd_demo_id` as the target primary key. Create a crosswalk
instead. Demos also generates `medicaid_id` and `chip_id` with triggers, so the
PMDA demo number belongs in the migration crosswalk/audit data, not in those
generated columns.
+
[source,sql]
----
...
. Load demo child data: contacts, comments, statuses, dates, authorities,
files, site visits, PAFs.
+
For demo child data, the key rule is: every extracted row should carry
`pmda_mdcd_demo_id`, and every target-side load should join through
`migration_pmda_demo_xref`. Do not join child data by state/name/demo number
once the crosswalk exists.
+
[source,sql]
----
...
. Load programs and demo-type-specific details.
+
Programs are the parent rows in `mdcd_pgm`. Demo types are not stored in one
table; PMDA spreads them across many `+mdcd_*_pgm_dtl+` tables and normalizes
them for the UI with `v_demo_mgmt_demo_types`. Use the view for
coverage/reconciliation, but keep the underlying detail-table rows if Demos
needs full source fidelity.
+
[source,sql]
----
...
. Load deliverables with their statuses, comments, files, due-date requests,
and history.
+
Deliverables are one of the highest-risk migration areas. `mdcd_dlvrbl` is only
the parent row. Status history, comments, uploaded files, BN/MRT flags,
due-date-change requests, and paper records all affect what users see.
+
[source,sql]
----
...
. Load pending application management data separately from approved demos.
+
Pending application management is not just another view of `mdcd_demo`. New
demonstrations, amendments, and extensions live around `mdcd_pendg_demo` +
`mdcd_demo_aplctn`; amendments/extensions can point back to an approved PMDA
demo through `mdcd_demo_aplctn.mdcd_demo_id`. Keep this as a separate migration
stream and crosswalk it back to approved demos only when that source link
exists.
+
[source,sql]
----
...
. Load BN/MRT/STC only if those workflows are expected to work in Demos.
+
Treat these as workflow migrations, not simple file copies. If Demos only needs
document history, migrate the file rows and source filenames. If Demos needs
the old PMDA feature behavior, also migrate the parsed workbook/report tables
keyed by `mdcd_demo_id`, `mdcd_dlvrbl_id`, `mdcd_dlvrbl_fil_doc_id`, and the
relevant template file id.
+
[source,sql]
----
...
. Reconcile by counts per demo/state: demos, programs, deliverables, files,
comments, pending apps, BN files, MRT files, STC docs.
+
[source,sql]
----
...
The biggest warning: `dltd_ind = 0` gives you active rows, but deleted/history
rows may still matter for audit, status trails, comments, and file provenance.

Also, PMDA has many "metadata-looking" reference codes that are actually
control flow, especially deliverable statuses, app statuses, demo types,
document types, and comment origins.

== Related

* xref:../sme/reference-pending-approved-rules.adoc[Pending/approved rules] --
  carries the workflow-level scope dispositions keyed to workflows 1 to 11.
* xref:explanation-migration-feasibility.adoc[What can migrate at all] -- the
  target-side feasibility view of the same source surface.
* xref:reference-cross-cutting-derivations.adoc[Cross-cutting derivations] --
  how PMDA source columns become DEMOS values.
* xref:reference-users-person-migration.adoc[Users and person migration] -- the
  built form of workflow 2.
* xref:reference-demonstration-role-migration.adoc[Demonstration role
  assignment migration] -- the built form of the contact slots in workflow 3.
