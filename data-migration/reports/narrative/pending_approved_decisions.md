...
PMDA is workflow-centric. The eleven numbered workflows below are the ones
described in `docs/developer/reference-legacy-pmda-workflows.adoc`, which is
the surviving record of the source application's shape (reconstructed from
PMDA's `cma-site` and `cma-service` repositories; anchors `#workflow-1`
through `#workflow-11`). The decision table above records *entity* rules;
...
target model" in `docs/developer/explanation-migration-feasibility.adoc`).
...
priority ranking of 2026-07-21 (see §11 of
`reports/narrative/sme_signoff_2026-07-20.md`) puts `deliverable_action` in his
lowest band and accepts a read-only snapshot for extensions/resubmit requests
(D12). MINIMAL (seeded genesis + one current-status
