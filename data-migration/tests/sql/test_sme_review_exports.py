...
def _provision_othr_held(conn: Any) -> None:
    ...
    conn.execute(
        ...
        "SELECT * FROM (VALUES "
        f"  (11, 1, '{DEMO_UUID}'::uuid, 'Zeta Waiver', 'free-text name is not a seeded tag'),"
        f"  (10, 1, '{DEMO_UUID}'::uuid, 'Alpha Waiver', 'parent demonstration not migrated')"
        ") AS v(legacy_id, legacy_demo_id, demonstration_id, othr_name, reason)"
    )
...
def test_othr_held_rows_ordered(pg_db: psycopg.Connection) -> None:
    ...
    assert rows[0]["demonstration_id"] == DEMO_UUID
