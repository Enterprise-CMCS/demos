...
WITH finalized_demos AS (
    SELECT
        ...
        expiration_date
    FROM
        {{ source('legacy_pmda_staged', 'final_demos_app_demonstration') }}
    WHERE
        status_id = 'Approved'
),
...
LEFT JOIN
    {{ source('legacy_pmda_staged', 'crosswalk_mdcd_dlvrbl_type_cd_to_deliverable_type_id') }} AS cw1
    ON
...
LEFT JOIN
    {{ source('legacy_pmda_staged','final_demos_app_person') }} AS f_person
    ON
