...
FROM {{ ref('deliverables_active_due_date_change_request') }} AS deliverable_extension
WHERE
    ...
    AND deliverable_extension._legacy_mdcd_due_dt_chg_rqst_id NOT IN (
        SELECT e4._legacy_mdcd_due_dt_chg_rqst_id
        FROM {{ ref('errors_deliverable_extension_with_final_date_granted') }} AS e4
    )
