...
BEGIN
    IF TG_OP IN ('INSERT', 'UPDATE') THEN
        INSERT INTO demos_app.uipath_result_history (
            revision_type,
            id,
            request_id,
            response,
...
BEGIN
    IF TG_OP IN ('INSERT', 'UPDATE') THEN
        INSERT INTO demos_app.uipath_value_history (
            revision_type,
            id,
            uipath_result_id,
            document_id,
        ...
        VALUES (
            ...
            END,
            NEW.id,
            NEW.uipath_result_id,
            NEW.document_id,
            NEW.application_id,
            NEW.field_id,
            NEW.value,
            NEW.text_length,
            NEW.text_start_index,
            NEW.confidence,
            NEW.token_list,
            NEW.created_at,
            NEW.updated_at
    ...
    ELSIF TG_OP = 'DELETE' THEN
        ...
        VALUES (
            'D'::demos_app.revision_type_enum,
            OLD.id,
            OLD.uipath_result_id,
            OLD.document_id,
            OLD.application_id,
            OLD.field_id,
            OLD.value,
            OLD.text_length,
            OLD.text_start_index,
            OLD.confidence,
            OLD.token_list,
            OLD.created_at,
            OLD.updated_at
...
CREATE OR REPLACE FUNCTION demos_app.log_changes_user_action()
RETURNS TRIGGER AS $$
BEGIN
    IF TG_OP IN ('INSERT', 'UPDATE') THEN
        INSERT INTO demos_app.user_action_history (
            revision_type,
            id,
            action_timestamp,
            user_id,
...
CREATE OR REPLACE TRIGGER log_changes_user_action
AFTER INSERT OR UPDATE OR DELETE ON demos_app.user_action
FOR EACH ROW EXECUTE FUNCTION demos_app.log_changes_user_action();
