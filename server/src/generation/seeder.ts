...
import { assignPersonToState } from "./assignPersonToState.js";
import { generateDemonstration } from "./generateDemonstration.js";
...
async function seedUsers(userCount: number) {
  console.log("🌱 Seeding users...");
  for (let i = 0; i < userCount; i++) {
    generateUser({
      firstName: faker.person.firstName(),
      lastName: faker.person.lastName(),
      personTypeId: PERSON_TYPES[i % PERSON_TYPES.length],
    });
  }
}

async function assignAllPeopleToStates() {
  console.log("🌱 Assigning people to states...");
  const allPeople = await prisma().person.findMany();
  for (let i = 0; i < allPeople.length; i++) {
    const stateId = STATES_AND_TERRITORIES[i % STATES_AND_TERRITORIES.length].id;
    assignPersonToState(allPeople[i].id, stateId);
  }
}

async function seedDemonstrations(demonstrationCount: number) {
  console.log("🌱 Seeding demonstrations...");
  const projectOfficerUsers = await prisma().user.findMany({
    where: { personTypeId: "demos-cms-user" },
  });
  for (let i = 0; i < demonstrationCount; i++) {
    generateDemonstration({
      name: `Demonstration ${i + 1}`,
      state: STATES_AND_TERRITORIES[i % STATES_AND_TERRITORIES.length].id,
      sdgDivision: SDG_DIVISIONS[i % SDG_DIVISIONS.length],
      projectOfficerUserId: projectOfficerUsers[i % projectOfficerUsers.length].id,
    });
  }
}
...
async function seedDatabase() {
  ...
  seedUsers(50);
  assignAllPeopleToStates();
  seedDemonstrations(20);
