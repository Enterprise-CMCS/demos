...
import { validateCreateAmendmentInput } from "./validateAmendmentInputs";
...
describe("validateAmendmentInputs", () => {
  ...
  describe("validateCreateAmendmentInput", () => {
    ...
    it("should not throw if none of the rules are violated", async () => {
      ...
      await expect(
        validateCreateAmendmentInput(testInput, mockTransaction)
      ).resolves.toBeUndefined();
    });

    it("should get the demonstration info", async () => {
      await validateCreateAmendmentInput(testInput, mockTransaction);
    ...
    it("should throw if the demonstration status check fails", async () => {
      ...
      try {
        await validateCreateAmendmentInput(testInput, mockTransaction);
        throw new Error("Expected validateCreateAmendmentInput to throw, but it did not.");
      } catch (e) {
        expect(e).toBeInstanceOf(GraphQLError);
        const error = e as GraphQLError;
        expect(error.message).toBe(
          "One or more validation checks for createAmendment have failed."
        );
        expect(error.extensions.code).toBe("CREATE_AMENDMENT_VALIDATION_FAILED");
