/*
  ...
  - A unique constraint covering the columns `[id,grantLevelId]` on the table `role` will be added. If there are existing duplicate values, this will fail.
  ...
  - Added the required column `grantLevelId` to the `role` table without a default value. This is not possible if the table is not empty.
...
-- CreateIndex
CREATE UNIQUE INDEX "role_id_grantLevelId_key" ON "demos_app"."role"("id", "grantLevelId");
...
-- AddForeignKey
ALTER TABLE "demos_app"."role" ADD CONSTRAINT "role_grantLevelId_fkey" FOREIGN KEY ("grantLevelId") REFERENCES "demos_app"."grant_level"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "demos_app"."role_permission" ADD CONSTRAINT "role_permission_role_id_grant_level_id_fkey" FOREIGN KEY ("role_id", "grant_level_id") REFERENCES "demos_app"."role"("id", "grantLevelId") ON DELETE RESTRICT ON UPDATE CASCADE;
