...
describe("getRecipients", () => {
  ...
  it("queries recipients for the given deliverable id", async () => {
    await getRecipients(client, deliverableId);

    expect(query).toHaveBeenCalledExactlyOnceWith(EMAIL_RECIPIENTS_QUERY, [deliverableId]);
  });

  it("returns the query result rows", async () => {
    const result = await getRecipients(client, deliverableId);
