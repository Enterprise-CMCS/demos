...
export function renderMultipleDeliverablesCreatedEmail(rawPayload: unknown): EmailTemplateResult {
  ...
  const firstDeliverable = getRequiredObject(deliverables[0], "deliverables[0]", emailType);
  ...
  const validatedDeliverables = deliverables.map((value, index) => {
    const deliverable = getRequiredObject(value, `deliverables[${index}]`, emailType);
    ...
    const id = getRequiredString(deliverable.id, `deliverables[${index}].id`, emailType);

    return {
      dueDate: formatDate(
        getRequiredString(deliverable.dueDate, `deliverables[${index}].dueDate`, emailType)
      ),
      link: `${getDemosAppUrl()}/deliverables/${id}`,
      name: getRequiredString(deliverable.name, `deliverables[${index}].name`, emailType),
    };
  });

  const demonstrationTitle = getRequiredString(demonstration.name, "demonstration.name", emailType);
    ...
    content: (
      <EmailLayout>
        ...
        <Text style={textStyle}>
          You have been assigned new {deliverableType} deliverables for your Demonstration. View
          these deliverables and any required next steps in the DEMOS system:
        </Text>
        ...
        <Text style={detailStyle}>
          Deliverables: {validatedDeliverables.map(({ name }) => name).join(", ")}
        </Text>
